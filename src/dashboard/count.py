...
LOCATION_UNKNOWN = 0
LOCATION_OUTSIDE = 1
LOCATION_IN_CAMP = 100
LOCATION_IN_TUNNEL = 1000

LOCATION_IN_TUNNEL_LIST = [ACCESS_1_ID, ACCESS_2_ID]
LOCATION_IN_CAMP_LIST = [CAMP_A_B_ID, CAMP_C_ID, CAMP_D_ID]
...
def __sc_set_access_point(ap, device_id):
  if ap == ACCESS_1_ID:
    ...
    S_CHECKING_DEVICE_LIST.add(device_id)
    S_MUCK_DEVICE_LIST.add(device_id)
  else:
    if device_id in AT_1_DEVICE_LIST:
      ...
      S_MUCK_DEVICE_LIST.remove(device_id)
      if not MUCK_DEVICE_LIST:
        clear_keys_of_sc(MUCK_ID)
...
def _delete_device_of_scanner(device_id):
  ...
  if device_id in S_AT_2_DEVICE_LIST:
    S_AT_2_DEVICE_LIST.remove(device_id)
  if device_id in S_MUCK_DEVICE_LIST:
    S_MUCK_DEVICE_LIST.remove(device_id)
    ...
    if device_id in OUT_LIST:
      if WORKER_COUNT.has_data(key, user_id):
        ...
        logging.info("%s(%s) check out. key :%s, device_id : %s, device_name : %s",
                     user_name, user_id, key, device_id, device_name)
  ...
  if not EMERGENCY[IS_EMERGENCY_KEY]:
    if hid in S_CHECKING_DEVICE_LIST:
      ...
      for gid in gid_set:
        ...
        elif not EXPIRE_CACHE.exists(gid) and hid in S_MUCK_DEVICE_LIST:
          _set_equip_count(MUCK_ID, org_id, gid, hid)
